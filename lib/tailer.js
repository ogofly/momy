...
class Tailer {
  ...
  importCollection (def) {
    ...
    return new Promise(resolve =>
      mongodb.connect(this.url, { })
        .then(cl => {
          const stream = cl.db(this.dbName).collection(def.name).find().stream()
          stream
            .on('data', item => {
              stream.pause()
  ...
  updateTimestamp (ts, skipUpdateMySQL) {
    ...
    return new Promise(resolve =>
      mongodb.connect(this.url2, {  })
        .then(cl =>
          cl.db('local').collection('oplog.rs').find().sort({ $natural: -1 }).limit(1)
            .next()
            .then(item => {
              if (item == null) return
              ts = item.ts.toNumber()
  ...
  tail () {
    ...
    return new Promise((resolve, reject) =>
      mongodb.connect(this.url2, ).then(cl => {
        this.db = cl
        const stream = cl.db('local').collection('oplog.rs').find(filters, curOpts).stream()
        stream
          .on('data', log => {
  ...
  process (log) {
    ...
    switch (log.op) {
      ...
      case 'u':
        const o = log.o.diff.u
        o[def.idName] = log.o2[def.idName]
        if (log.o.$set || log.o.$unset) {
          util.log(`Update a record in ${def.ns} (${def.idName}=${log.o2[def.idName]})`)
          return this.mysql.update(def, log.o2[def.idName], o, log.o.$unset)
        } else {
          const replaceFlag = true
          util.log(`Replace a record in ${def.ns} (${def.idName}=${log.o2[def.idName]})`)
          return this.mysql.insert(def, o, replaceFlag)
        }
      case 'd':
